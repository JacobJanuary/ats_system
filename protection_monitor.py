...
class ProtectionMonitor:
    ...
    def _calculate_breakeven_price(self, entry_price: float, side: str, exchange: Optional[BinanceExchange | BybitExchange] = None, symbol: str = "") -> float:
    ...
    async def _handle_position_duration_limit(self, exchange: BinanceExchange | BybitExchange, position: Dict,
        ...
        try:
            ...
            # Получаем tick_size в зависимости от биржи
            if isinstance(exchange, BinanceExchange):
            ...
            is_close_order_placed = self._check_existing_close_order(exchange, symbol, breakeven_price, tick_size, open_orders)
            ...
            # Проверяем rate limit перед отменой ордеров
            exchange_key = 'binance' if isinstance(exchange, BinanceExchange) else 'bybit'
            if not await self.rate_limiter.acquire(exchange_key, 'order', f'cancel_orders_{symbol}'):
                logger.warning(f"Rate limit exceeded for {exchange_key} cancel_orders_{symbol}")
                return
            
            # Отменяем обычные ордера
            await exchange.cancel_all_open_orders(symbol)
            
            # Для Bybit также отменяем position-level trading stops (TL/SL/TP)
            if isinstance(exchange, BybitExchange):
                if hasattr(exchange, 'cancel_position_trading_stops'):
                    await exchange.cancel_position_trading_stops(symbol)
                    logger.info(f"   Cancelled position-level trading stops for {symbol}")
            
            await self.rate_limiter.record_request(exchange_key, 'order', f'cancel_orders_{symbol}')
            ...
            if pnl >= 0:
                ...
                # Проверяем rate limit перед закрытием позиции
                if not await self.rate_limiter.acquire(exchange_key, 'order', f'close_position_{symbol}'):
                    logger.warning(f"Rate limit exceeded for {exchange_key} close_position_{symbol}")
                    return
                
                if await exchange.close_position(symbol):
                    await self.rate_limiter.record_request(exchange_key, 'order', f'close_position_{symbol}')
                    self.stats['positions_closed'] += 1
            else:
                logger.info(
                    f"   Position is at a loss. Setting final breakeven limit order for {symbol} at ${breakeven_price:.4f}")
                
                # Проверяем rate limit перед созданием лимитного ордера
                if not await self.rate_limiter.acquire(exchange_key, 'order', f'create_limit_{symbol}'):
                    logger.warning(f"Rate limit exceeded for {exchange_key} create_limit_{symbol}")
                    return
                ...
                await self.rate_limiter.record_request(exchange_key, 'order', f'create_limit_{symbol}')
        except Exception as e:
            logger.error(f"Failed to handle duration limit for {symbol}: {e}", exc_info=True)
            self.stats['errors'] += 1

    def _check_existing_close_order(self, exchange, symbol: str, breakeven_price: float, tick_size: float, open_orders: List[Dict]) -> bool:
        """Проверяет, есть ли уже ордер на закрытие позиции в безубыток"""
        ...
        for order in open_orders:
            ...
            # Проверяем reduceOnly
            reduce_only = False
            if exchange_name == 'BinanceExchange':
                reduce_only_value = order.get('reduceOnly', False)
                # Handle both boolean and string values
                if isinstance(reduce_only_value, bool):
                    reduce_only = reduce_only_value
                else:
                    reduce_only = str(reduce_only_value).lower() == 'true'
            else:  # BybitExchange
                reduce_only_value = order.get('reduceOnly', '')
                if isinstance(reduce_only_value, bool):
                    reduce_only = reduce_only_value
                else:
                    reduce_only = str(reduce_only_value).lower() == 'true'
            ...
            try:
                ...
                if abs(order_price - breakeven_price) <= tick_size:
                    return True
            except (ValueError, TypeError):
                continue  # Пропускаем ордер с некорректной ценой
    ...
    async def _is_protection_incomplete(self, exchange_name: str, pos: Dict, open_orders: List[Dict]) -> bool:
        ...
        try:
            ...
            if protection_complete:
                logger.debug(f"✅ Protection complete for {exchange_name} {symbol}: SL={has_sl}, TP={has_tp}, TS={has_ts}")
            else:
                logger.info(f"🛡️ Protection incomplete for {exchange_name} {symbol}: SL={has_sl}, TP={has_tp}, TS={has_ts}")
    ...
    async def _process_single_position(self, exchange_name: str, pos: Dict, orders_by_symbol: Dict[str, List[Dict]]):
        ...
        try:
            ...
            # ДОПОЛНИТЕЛЬНОЕ ЛОГИРОВАНИЕ ДЛЯ ДИАГНОСТИКИ
            logger.info(f"🔍 DEBUG: Processing {exchange_name} {symbol} position")
            logger.info(f"🔍 DEBUG: max_position_duration_hours = {self.max_position_duration_hours}")
            logger.info(f"🔍 DEBUG: Condition check: {self.max_position_duration_hours} > 0 = {self.max_position_duration_hours > 0}")
            
            # Проверяем превышение максимальной длительности позиции
            if self.max_position_duration_hours > 0:
                logger.info(f"🔍 DEBUG: Age calculation condition PASSED for {exchange_name} {symbol}")
                # УЛУЧШЕННЫЙ расчет возраста позиции
                age_hours = self._calculate_position_age(pos, exchange_name)
                
                if age_hours > self.max_position_duration_hours:
                    logger.warning(f"⏰ {exchange_name} {symbol} exceeded max duration: {age_hours:.1f}h > {self.max_position_duration_hours}h")
                ...
                logger.warning(f"⚠️ DEBUG: Age calculation condition FAILED for {exchange_name} {symbol} - max_position_duration_hours = {self.max_position_duration_hours}")
        ...
        # 🔍 DEBUG: Проверяем, что статистика вызывается
        print(f"🔍 ABSOLUTE DEBUG: About to call _log_exchange_statistics for {exchange_name}")
        print(f"🔍 ABSOLUTE DEBUG: Exchange stats keys: {list(self.exchange_stats.keys())}")
        print(f"🔍 ABSOLUTE DEBUG: {exchange_name} stats: {self.exchange_stats.get(exchange_name, 'NOT FOUND')}")
        
        logger.warning(f"🔍 DEBUG: About to call _log_exchange_statistics for {exchange_name}")
        logger.warning(f"🔍 DEBUG: Exchange stats for {exchange_name}: {self.exchange_stats[exchange_name]}")
        
        # ВЫВОДИМ СТАТИСТИКУ ПО ЗАВЕРШЕНИИ ЦИКЛА
        try:
            print(f"🔍 ABSOLUTE DEBUG: Calling _log_exchange_statistics for {exchange_name}")
            self._log_exchange_statistics(exchange_name)
            print(f"✅ ABSOLUTE DEBUG: _log_exchange_statistics completed for {exchange_name}")
            logger.warning(f"✅ DEBUG: _log_exchange_statistics completed successfully for {exchange_name}")
        except Exception as e:
            print(f"❌ ABSOLUTE DEBUG: Exception in _log_exchange_statistics: {e}")
            logger.error(f"❌ CRITICAL: Exception in _log_exchange_statistics for {exchange_name}: {e}", exc_info=True)
    ...
    def _calculate_position_age(self, position, exchange_name):
        """Calculate position age in hours"""
        try:
            if exchange_name == "Binance":
                # Binance typically uses updateTime or time fields
                timestamp = position.get("updateTime")
                if not timestamp:
                    # Fallback to current time if no timestamp
                    return 0.0
                # Convert milliseconds to seconds if needed
                if timestamp > 1e10:  # If it's in milliseconds
                    timestamp = timestamp / 1000
            else:  # Bybit
                # Bybit typically uses updatedTime or similar
                timestamp = position.get("updatedTime") or position.get("updateTime")
                if not timestamp:
                    # Fallback to current time if no timestamp
                    return 0.0
                # Convert milliseconds to seconds if needed
            ...
            return max(0.0, age_hours)  # Ensure non-negative
        except Exception as e:
